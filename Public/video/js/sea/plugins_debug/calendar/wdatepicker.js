var $dp
define('./wdatepicker', [], function(require, exports, module) {
	var WdatePicker;
	var _ = {
		$wdate : true,
		$dpPath : "",
		$crossFrame : window.frameElement==null?false:(window.frameElement.getAttribute('dialogIframe')==='true'?true:false),
	...
	function L() {
		...
		_ = module.id;// $[B].src.substring(0,
						// $[B].src.toLowerCase().indexOf("wdatepicker.js"));
		A = _.lastIndexOf("/");
		// if (A > 0){
		_ = _.substring(0, A + 1);
		// }if (_)
	...
	function N($) {
		$ = $ || V;
		var B = $[O], A = ($.innerWidth) ? $.innerWidth : (B[J] && B[J].clientWidth) ? B[J].clientWidth : B.body.offsetWidth, _ = ($.innerHeight)
				? $.innerHeight
	...
	function K(H, $) {
		...
		if (!$dp.dd
				|| $dp.eCont
				|| ($dp.lang && $dp.realLang && $dp.realLang.name != $dp.lang && $dp.getLangIndex && $dp.getLangIndex($dp.lang) >= 0)) {
			if ($dp.dd && !$dp.eCont)

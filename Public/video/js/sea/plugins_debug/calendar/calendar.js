...
function My97DP() {
	...
	if (getNP().notShowSS) {
		$d.innerHTML = "<div id=dpTitle><div class=\"navImg NavImgll\"><a href=\"###\"></a></div><div class=\"navImg NavImgl\"><a href=\"###\"></a></div><div style=\"float:left\"><div class=\"menuSel MMenu\"></div><input class=yminput></div><div style=\"float:left\"><div class=\"menuSel YMenu\"></div><input class=yminput></div><div class=\"navImg NavImgrr\"><a href=\"###\"></a></div><div class=\"navImg NavImgr\"><a href=\"###\"></a></div><div style=\"float:right\"></div></div><div style=\"position:absolute;overflow:hidden\"></div><div></div><div id=dpTime><div class=\"menuSel hhMenu\"></div><div class=\"menuSel mmMenu\"></div><div class=\"menuSel ssMenu\"></div><table cellspacing=0 cellpadding=0 border=0><tr><td rowspan=2><span id=dpTimeStr></span>&nbsp;<input class=tB maxlength=2><input value=\":\" class=tm readonly><input class=tE maxlength=2><input value=\":\" style='display:none;' class=tm readonly><input  style='display:none;' class=tE maxlength=2></td><td><button id=dpTimeUp></button></td></tr><tr><td><button id=dpTimeDown></button></td></tr></table></div><div id=dpQS></div><div id=dpControl><input class=dpButton id=dpClearInput type=button><input class=dpButton id=dpTodayInput type=button><input class=dpButton id=dpOkInput type=button></div>";
	} else {
...
My97DP.prototype = {
	init : function() {
		...
		this.minDate = this.doCustomDate($dp.minDate, $dp.minDate != $dp.defMinDate ? $dp.realFmt : $dp.realFullFmt,
				$dp.defMinDate);
		this.maxDate = this.doCustomDate($dp.maxDate, $dp.maxDate != $dp.defMaxDate ? $dp.realFmt : $dp.realFullFmt,
				$dp.defMaxDate);
		...
		getNP().init && getNP().init($dp, $d, sv, _setAll);// nihf
	},
	...
	_fd : function() {
		var E, C, D, K, A, H = new sb(), F = $lang.aWeekStr, G = $dp.firstDayOfWeek, I = "", $ = "", _ = new DPDate($dt.y,
				$dt.M, $dt.d, 0, 0, 0), J = _.y, B = _.M;
			...
			for (D = 0; D < 7; D++) {
				...
				if (K) {
					if (this.checkValid(_, "d", D)) {
						if (this.testSpeDay(Math.abs((new Date(_.y, _.M - 1, _.d).getDay() - $dp.firstDayOfWeek) % 7))
								|| this.testSpeDate(_))
							I = "WspecialDay";
	...
	_f : function(p, c, r, e, isR) {
		...
		if (p == 'H' && getNP().schedule_24 == true) {// nihf
			s.a("<tr nowrap=\"nowrap\">");
			s.a("</tr>");
			s.a("<td nowrap ");
			$dt[p] = 24;
			if (($dp.opposite && this.checkRange($dt, p) == 0) || this.checkValid($dt, p)) {
				s.a("class='menu' onmouseover=\"this.className='menuOn'\" onmouseout=\"this.className='menu'\" onmousedown=\"");
				s.a("hide($d." + p + "D);$d." + fp + "I.value=24;$d." + fp + "I.blur();\"")
			} else {
				s.a("class='invalidMenu'");
			}
			s.a(">24</td>");
			s
					.a("<td nowrap>&nbsp;</td><td nowrap>&nbsp;</td><td nowrap>&nbsp;</td><td nowrap>&nbsp;</td><td nowrap>&nbsp;</td>");
			s.a("</tr>")
		}
		s.a("</table>");
		$dt[p] = bak;
	...
	_fHMS : function(A, _, $) {
		var el = $d[A + "D"];
		if (getNP().schedule_24 && A == 'H') {
			el.style.cssText = 'display: block;margin-top:-107px;';
		}
		el.innerHTML = this._f(A, 6, _, $)
	},
	_fH : function() {
	...
	_fillQS : function(A) {
		...
		for (var B = 0; B < _.length; B++)
			if (_[B]) {
				...
				if (getNP().schedule_24 && _[B].H == 0 && _[B].m == 0) {
					_[B].H = 24;
				}
	...
	getP : function(D, _, $) {
		...
		switch (D) {
			...
			case "y" :
				...
				_ = _ || C[0];
				F = function(_) {
					return doStr((_.length < 4) ? (_.length < 3 ? $.y % 100 : ($.y + 2000 - $dp.yearOffset) % 1000) : A,
							_.length)
				};
				break;
			case "M" :
				C = ["MMMM", "MMM", "MM", "M"];
				F = function($) {
					return ($.length == 4) ? $lang.aLongMonStr[A - 1] : ($.length == 3) ? $lang.aMonStr[A - 1] : doStr(A,
							$.length)
				};
				break
		}
	...
	autoSize : function() {
		...
		for (var C = 0; C < _.length; C++) {
			...
			if (_[C].contentWindow == window && A) {
				...
				if (B && $d.bDiv.style.display == "none" && $d.tDiv.style.display != "none"
	...
	pickDate : function() {
		...
		$sdt.loadFromDate($dt);
		this.update();
		if (getNP().schedule_24 == true) {// nihf
			sv("H", $dp.H);
			sv("m", $dp.m);
		}
		if (!$dp.eCont)
	...
	initQS : function() {
		...
		if (E > $)
			...
			//guodong 修改时间区间排序，下一行注释为原来写法；
			//C = [-60, -30, 0, 30, 60, -15, 15, -45, 45];
			C = [-60,-45,-30,-15, 0, 15, 30 ,45, 60];
		else
			for (H = 0; H < $; H++)
				C[H] = $dt[_] - 2 + H;
		for (H = G = 0; H < E; H++) {
...
function elFocus() {
	var _ = $dp.el;
	try {
		if (_.style.display != "none" && _.type != "hidden"
				&& (_.nodeName.toLowerCase() == "input" || _.nodeName.toLowerCase() == "textarea")) {
			if ($dp.srcEl == _)
				$dp.el["My97Mark"] = true;
			$dp.el.focus();
// return nihf
...
function setDisp(_, $) {
	for (i = 0; i < _.length; i++){
		_[i].style.display = $;
		if($=='none'&&_[i].id=='_my97DP'&&$dp.cal.oldValue != $dp.el[$dp.elProp]){// nihf
			callFunc("onchange");
		}
	}
}
...
function c(p, pv) {
	if (p == "M")
		...
		if (getNP().schedule_24 == true) {
			pv = makeInRange(pv, 0, 24);
			if (pv == 24) {
				sv("m", 0);
				$d["mI"].disabled = true;
			} else {
	...
	if (!callFunc(p + "changing")) {// $sdt[p] != pv && nihf
		var func = "sv(\"" + p + "\"," + pv + ")", rv = $c.checkRange();
		if(p=='H'||p=='m'){//by nihf
			var oldValue = $dp.cal.date[p];
			if (rv == 0)
				eval(func);
			rv = $c.checkRange();
			if(rv!=0){
...
function _tab(M) {
	...
	if ($dp.enableInputMask && !$OPERA && !$dp.readOnly && $c.currFocus == $dp.el && (Q >= 48 && Q <= 57)) {
		...
		while ((K = _.exec($dp.dateFmt)) !== null) {
			...
			if (I.arr[X + 1] || A == "M" && W > 12 || A == "d" && W > 31 || A == "H" && W > 23 || "ms".indexOf(A) >= 0
					&& W > 59) {
				if (K[0].length == 2)
					B = "0" + $;
				else

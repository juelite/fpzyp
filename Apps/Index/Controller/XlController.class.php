...
/**
 * 此注释用于后台自动生成权限表（勿删）
 * @ACTION_DEFAULT_CN_NAME:线缆工单
 * @ACTION_DEFAULT_ICON:menu-icon fa fa-google-wallet
 */
class XlController extends ActionController {
    ...
    public function  lockzyp(){
        ...
        $condition['id'] = $id;
        $obj = M('xl_order')->find($id);
        if($obj){
            if($obj['uid'] != $curUser['id']){
                $this->error('当前记录不属于你 !');
            }else{
                $flag = M('xl_order')->where($condition)->setField(array('status'=>1,'lock_time'=>time()));
                if($flag){
                    $this->success('结项成功!');
                }
            }
    ...
    public function zyp(){
        ...
        if($act === 'delZyp'){
            $id = I('get.id');
            $this->delZypAndSub(M('xl_order'),$id);
        }
        $field = 'wt_xl_order.id ,
        wt_admin.username as username ,
        wt_xl_order.gongchengmingcheng as gongchengmingcheng,
        wt_xl_order.gongzuobanzu as gongzuobanzu,
        wt_xl_order.gongdanbianhao as gongdanbianhao,
        wt_xl_order.shigongfuzeren as shigongfuzeren,
        wt_xl_order.jihuagongzuoshijian as jihuagongzuoshijian,
        wt_xl_order.jihuagongzuoshijian_kaishi as jihuagongzuoshijian_kaishi,
        wt_xl_order.jihuagongzuoshijian_jieshu as jihuagongzuoshijian_jieshu,
        wt_xl_order.status,
        wt_xl_order.lock_time
        ';
        $curUser = session('curUser');
        $curUserId = $curUser['id'];
        ...
        ->field($field)
        ->where($condition)
        ->join('left join wt_admin on wt_admin.id = wt_xl_order.uid')
        ->count();
        $page = new \Think\Page($count, C("PAGE_LIMITS"));
        $show = $page->show();
         ...
         ############################
        $orderStr = '';
        if(isset($_COOKIE['sort3'])){
            foreach($_COOKIE['sort3'] as $k => $v){
    ...
    public function editzyp(){
        if(IS_POST){
            ...
            $curUserId = $curUser['id'];
            $condition['uid'] = $curUserId;
            $record = M('xl_order')->where($condition)->find($id);
            if(!$record){
                $this->error("找不到你要的数据，或数据不属于你");
            }
    ...
    public function delzyp(){
        ...
        if($obj){
            if($obj['uid'] != $curUser['id']){
                ...
                $flag = M('xl_order')->where($condition)->setField('status' , -1);
                if($flag){
                    $this->success('作废成功!');
                }else{
                    $this->error("当前记录不属于你 !");
...
##
            ...
            $tmp = $this->get_zyp_sub_order('xl_order',$id);
            $this->tmp = $tmp;
            $this->display();
        }
